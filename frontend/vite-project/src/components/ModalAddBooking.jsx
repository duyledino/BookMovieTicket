...
function ModalAddBooking({ isOpen, onClose, setIsChange }) {
  ...
  useEffect(() => {
    const fetchCalendar = async () => {
      const res = await axios.get(
        `http://localhost:8000/api/v1/calendar/getAllCalendarFromASpecificFilm?film_id=${filmID}`
      );
      console.log(res);
  ...
  const onCreate = () => {
    const object ={
      ...
      popCorn: popCornBook.length===0?[]:popCornBook.map((pop) => ({
        ...
        bookFrequent: Number(numberPopCorn),
        total_price: Number(pop.price) * Number(numberPopCorn),
      })),
    }
    console.log(object);
    const res = axios.post(
      `http://localhost:8000/api/v1/booking/createBooking`,object
          ...
          >
            ...
            <div className="flex items-center gap-3 w-full">
              ...
              >
                ...
                {users.length > 0 &&
                  users.map((user) => (
                    <option value={user.customer_id}>{user.username}</option>
                  ))}
                {/* Customers will be loaded from DB */}
              </select>
            </div>
