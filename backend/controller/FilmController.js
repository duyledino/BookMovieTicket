...
const getLastestFilm = async(req,res)=>{
  ...
  if (latestFilm.length === 0)
    return res.status(202).json({ Message: "No Film found.", latestFilm: latestFilm });
...
const getComingSoonFilmLimit4 = async(req,res)=>{
  ...
  if(comingSoon.length===0) return res.status(202).json({Message: "No Film found."});
...
const getAllComingSoonFilm = async (req, res) => {
  ...
  if (allComingSoon.length === 0) return res.status(202).json({ Message: "No Film found." });
...
const getNowFilmLimit4 = async(req,res)=>{
  ...
  if(NowFilm.length===0) return res.status(202).json({Message: "No Film found."});
...
const updateAFilm = async (req, res) => {
  ...
  res.status(200).json({ Message: "Update Film successfully", updated: updateFilm });
};
