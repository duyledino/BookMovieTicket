...
function AddToStoreButton() {
  ...
  const handleClick = async () => {
    if (Object.keys(detail).length !== 0) {
      ...
      if (resFilm.status === 200) console.log("create film successfully.");
      else if (resFilm.status === 400) console.log("failed to create film.");
    }
  };
  return (
    <>
      <button
        className={`bg-amber-400 text-amber-50 p-2 hover:rounded-xl transition-all cursor-pointer ${Object.keys(detail).length===0?"w-full m-auto max-w-[300px]":"max-w-[300px]"}`}
        onClick={handleClick}
      >
        Add To Store
      </button>
