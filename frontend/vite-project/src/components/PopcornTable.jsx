  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (res.status === 200) setPopCorn(res.data.fixAllpopcorn);
  ...
  return (
    <>
      ...
      <div className='flex flex-wrap lg:flex-row flex-col lg:justify-normal justify-center items-center gap-3.5 min-w-full p-2'>
        {popCorn!==undefined && popCorn.length > 0 ? popCorn.map((item,index)=>(
            <Popcorn  isClick={isClick} handleClick={handleClick} popCornId={item.popcorn_id} key={item.popcorn_id} booked={item.booked} name={item.name} price={item.price} total={item.total} base64Image={item.base64Image} />
        )) : ""}
      </div>
    </>
  );
