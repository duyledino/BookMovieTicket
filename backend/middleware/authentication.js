...
const createToken = (user, res) => {
  ...
  res.cookie("token", token, {
    ...
    secure: process.env.NODE_ENV === "production", // must be true on HTTPS in production
...
const authenticateUser = (req, res, next) => {
  ...
  console.log("decode in authenticateUser;",decode);
  if (!decode)
    return res.status(203).json({ Message: "Expire or wrong token" });
  req.user = decode;
  next();
};
